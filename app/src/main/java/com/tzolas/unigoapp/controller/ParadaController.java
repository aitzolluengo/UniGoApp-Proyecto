...
public class ParadaController {
    ...
    public static ParadaCercana obtenerParadaMasCercana(Context context, double latUser, double lonUser) {
        Map<String, LatLng> stopCoords = new java.util.HashMap<>();
        Map<String, String> stopNames = GtfsUtils.cargarStopNames(context, stopCoords);

        ParadaCercana masCercana = null;
        double minDist = Double.MAX_VALUE;

        for (String stopId : stopCoords.keySet()) {
            LatLng coords = stopCoords.get(stopId);
            double dist = ParadaUtils.calcularDistancia(latUser, lonUser, coords.latitude, coords.longitude);
            if (dist < minDist) {
                minDist = dist;
                String nombre = stopNames.getOrDefault(stopId, "Sin nombre");
                masCercana = new ParadaCercana(stopId, nombre, dist);
            }
        }
